...
where sb.s_id = s.seller_id and sb.w_id=w.warehouse_id and w.capacity=(select max(capacity)
    from warehouse)
;
...
where s.seller_id = sp.s_id and sp.p_id  = p.id and sb.cost = (select max(cost)
    from sells)
;
...
where o.p_id = c.customer_id and o.p_id=p.id
c.email='customer@gmail.com' ; 
